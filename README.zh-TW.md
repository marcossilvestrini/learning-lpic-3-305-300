...
客製化模板_Vagrantfile-主題-XXX_。該檔案包含實驗室的虛擬機器配置。例子：

-   文件[Vagrantfile-主題-351](./vagrant/Vagrantfile-topic-351)
    -   vm.clone_directory = "&lt;您的驅動程式字母>:\\<folder>\\&lt;to_machine>\\#{VM_NAME}-instance-1"
        範例： vm.clone_directory = "E:\\伺服器\\VMware\\#{VM_NAME}-instance-1"
    -   vm.vmx[“孟加斯”"]= ""
    -   虛擬機器vmx[“ numvcpus”"]= ""
...
切換_vagrantfile-topic-xxx_模板並複製一個帶有名稱的新文件_流浪文件_
...
如果你使用Windows平台，我為上下虛擬機器建立一個powershell腳本。
...
-   **完整的虛擬化：**虛擬機器可以運行未經修改的來賓作業系統，因為虛擬機器管理程式提供了硬體環境的完整模擬。
...
-   **虛擬機器：**可以運行未修改的客人操作系統。
-   **半虛擬化：**需要修改來賓操作系統以與管理程序一起使用。

###### 表現

-   **虛擬機器：**由於硬體輔助執行，通常可以提供接近本機的效能。
-   **寄生蟲：**可以透過減少硬體模擬的開銷來提供高效的效能，但依賴修改後的客戶作業系統。

###### 硬件依賴性

-   **虛擬機器：**需要特定的CPU功能（Intel VT-X，AMD-V）。
-   **半虛擬化：**不需要特定的CPU功能，而需要修改的訪客操作系統。
...
-   **半虛擬化：**需要對來賓操作系統進行其他設置和修改，從而提高複雜性。
...
1.  **本地和遠程內存**：每個處理器都有自己的本地內存，可以更快地存取該內存。然而，它也可以存取其他處理器的內存，儘管這需要更長的時間。
2.  **區分潛伏期**：記憶體存取的延遲取決於處理器是存取其本地記憶體還是另一個節點的記憶體。本地記憶體存取速度更快，而存取另一個節點（遠端）的記憶體速度較慢。
...
-   **由 Citrix 收購**：2007年，Xensource被Citrix Systems，Inc。收購。Citrix使用Xen Technology作為其Citrix Xenserver產品的基礎，該產品成為基於Xen的流行企業級虛擬化平台。
...
-   **藥丸和Hedools：**Xen 專案還包括用於管理 Xen 虛擬機器管理程式安裝的 XAPI (XenAPI) 等工具，以及用於系統管理和最佳化的各種其他實用程式。
...
-   **存取控制：**XAPI還提供訪問控制機制，以確保只有授權用戶才能在虛擬環境中執行特定操作。
...
-   **Xen來源：**Xen商業化的公司後來被Citrix收購，導致Citrix Xenserver的發展。
