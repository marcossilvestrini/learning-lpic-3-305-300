...
-   **建築圖像：**
    ...
    任何工具（Docker，Podman，buildah）都可以產生圖像**OCI圖像規格**因此，它們到處都是兼容。
-   **運行容器：**
...
-   **這是什麼？**實用程序**檢查，複製和移動容器圖像**在註冊表之間**無需拉或跑步**他們。
-   **使用：**移動圖像，檢查簽名和元數據。
-   **亮點：**沒有守護程序，非常適合自動化和安全性。
-   [更多信息](<>)
...
-   **這是什麼？****基於容器的虛擬化**Linux的解決方案（預先約會的現代容器工具）。
-   **使用：**輕巧的VPS（虛擬私人服務器）共享相同的內核。
-   **亮點：**非常有效，但不如VM隔離（股內核）。
...
-   **這是什麼？**用C編寫的容器的超快速，最小的OCI運行時（不去）。
-   **使用：**執行最小開銷的容器。
-   **亮點：**比Runc更快，更輕，在某些系統上，Podman的默認值。
...
-   **這是什麼？**結合容器和VM的開源項目：每個容器均在輕量級的微VM中運行。
-   **使用：**敏感工作負載或多租戶環境的強烈隔離。
-   **亮點：**VM級安全性，近乎使用者的性能。
...
| 專案          | 類別     | 隔離           | 守護程序？ | 主要用途    | 無根 | 筆記            |
| ----------- | ------ | ------------ | ----- | ------- | -- | ------------- |
| **Subman**  | 編排     | 容器           | 不     | 管理容器    | 是的 | 類似碼頭的CLI      |
| **Buildah** | 建造     | N/A。         | 不     | 構建圖像    | 是的 | 對於CI/CD，無容器運行 |
| **範圍**      | 圖像轉移   | N/A。         | 不     | 移動/檢查圖像 | 是的 | 無容器執行         |
| **OpenVZ**  | 虛擬化    | 容器/VPS       | 是的    | 輕量級vps  | 不  | 內核分享，傳統技術     |
| **克倫**      | OCI運行時 | 容器           | 不     | 快速容器運行時 | 是的 | 比runc快        |
| **單詞容器**    | 運行時/VM | 每個容器的microVM | 不     | 強烈的隔離   | 是的 | VM級安全性        |
...
-   [虛擬化和容器化](<>)
    ...
    -   [Docker中的Apparmor個人資料](https://docs.docker.com/engine/security/apparmor/)
    -   [Selinux](https://pt.wikipedia.org/wiki/SELinux)
    -   [Apparmor Selinux比較](https://www.redhat.com/en/blog/apparmor-selinux-isolation)
    ...
    -   [範圍](https://www.redhat.com/pt-br/topics/containers/what-is-skopeo)
    -   [Buildah](https://www.redhat.com/en/topics/containers/what-is-buildah)
    -   [OpenVZ](https://openvz.org/)
    -   [克倫](https://www.redhat.com/en/blog/introduction-crun)
    -   [說](https://katacontainers.io/)
-   [OpenStack文檔](<>)
