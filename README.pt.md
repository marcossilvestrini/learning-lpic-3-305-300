...
| **SELinux**  | Aplique o controle de acesso obrigatório com políticas de sistema rigorosas     |
...
Para testar o script docker, use o registro:[docker-registry-lab.sh](scripts/docker/docker-registry-lab.sh).
...
Para testar o docker compose, use exemplos de serviços em[aplicativos](apps/).

#### 🌐 Enxame Docker

![swarm-nodes](images/swarm-nodes.png)_Arquitetura Swarm com nós gerenciadores e trabalhadores_

![swarm-services](images/swarm-services.png)_Serviços Swarm com múltiplas réplicas_

Docker Swarm é a ferramenta de orquestração nativa do Docker que permite gerenciar um cluster de hosts Docker como um único sistema virtual. Facilita a implantação, o gerenciamento e o dimensionamento de aplicativos em contêineres em várias máquinas.

##### Conceitos-chave do Docker Swarm

| Conceito             | Descrição                                                                                                                                             |
| :------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------- |
| 🌐**Enxame**         | Um cluster de hosts Docker em execução no modo swarm.                                                                                                 |
| 🤖**Nó**             | Um host Docker participando do enxame. Os nós podem ser gerentes ou trabalhadores.                                                                    |
| 👑**Nó Gerenciador** | Responsável por gerenciar o estado do enxame, agendar tarefas e manter o estado desejado do cluster.                                                  |
| 👷**Nó de trabalho** | Executa tarefas atribuídas pelos nós gerenciadores, executando os contêineres reais.                                                                  |
| 🚀**Serviço**        | Uma definição abstrata de um recurso computacional (por exemplo, um servidor web Nginx) que pode ser dimensionado e atualizado de forma independente. |
| 📝**Tarefa**         | Um contêiner em execução que faz parte de um serviço.                                                                                                 |

##### ✨ Principais características

| Recurso                      | Descrição                                                                                                                                   |
| :--------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------ |
| ⬆️**Alta disponibilidade**   | Distribui serviços entre vários nós, garantindo que os aplicativos permaneçam disponíveis mesmo se um nó falhar.                            |
| ⚖️**Escalabilidade**         | Amplie ou reduza facilmente os serviços para lidar com cargas de trabalho variadas.                                                         |
| 🔄**Balanceamento de carga** | O balanceamento de carga integrado distribui as solicitações uniformemente entre as réplicas de serviço.                                    |
| 🚀**Atualizações contínuas** | Execute atualizações nos serviços sem tempo de inatividade.                                                                                 |
| 😊**Facilidade de uso**      | Integrado diretamente ao Docker Engine, tornando-o relativamente simples de configurar e gerenciar em comparação com outros orquestradores. |

##### 🐳 Comandos de gerenciamento de enxame

| Comando                    | Descrição                                                        | Exemplo                                               |
| :------------------------- | :--------------------------------------------------------------- | :---------------------------------------------------- |
| 👑**`docker swarm init`**  | Inicializa um novo enxame no nó atual, tornando-o o gerenciador. | `docker swarm init --advertise-addr 192.168.1.10`     |
| 👷**`docker swarm join`**  | Une um nó a um enxame existente como trabalhador ou gerente.     | `docker swarm join --token <TOKEN> 192.168.1.10:2377` |
| 👋**`docker swarm leave`** | Remove o nó atual do enxame.                                     | `docker swarm leave --force`                          |
| 📜**`docker node ls`**     | Lista todos os nós do enxame.                                    | `docker node ls`                                      |
...
| ⚖️**`docker service scale`**  | Dimensiona um ou mais serviços replicados. | `docker service scale web=5`                                   |
| 🔄**`docker service update`** | Atualiza a configuração de um serviço.     | `docker service update --image nginx:latest web`               |
| 🗑️**`docker service rm`**    | Remove um serviço do enxame.               | `docker service rm web`                                        |
| 📜**`docker service ls`**     | Lista todos os serviços do enxame.         | `docker service ls`                                            |
| 📝**`docker service ps`**     | Lista as tarefas de um ou mais serviços.   | `docker service ps web`                                        |

Para testar o script docker swarm, use:[docker-swarm.sh](scripts/docker/docker-swarm.sh).

#### ☸️ Kubernetes

Kubernetes, também conhecido como K8s, é uma plataforma de código aberto para automatizar a implantação, escalonamento e gerenciamento de aplicativos em contêineres. Ele agrupa contêineres que compõem um aplicativo em unidades lógicas para fácil gerenciamento e descoberta.

##### 🏛️ Arquitetura Kubernetes

![Kubernetes Architecture](images/k8s-components.png)

Um cluster Kubernetes consiste em um conjunto de máquinas de trabalho, chamadas**nós**, que executam aplicativos em contêineres. Cada cluster possui pelo menos um nó de trabalho. O(s) nó(s) de trabalho hospeda(m) o(s)**Vagens**quais são os componentes da carga de trabalho do aplicativo. O**plano de controle**gerencia os nós do trabalhador e os pods no cluster.

###### ✈️ Componentes do plano de controle

Os componentes do plano de controle tomam decisões globais sobre o cluster (por exemplo, agendamento), bem como detectam e respondem a eventos de cluster.

| Componente                              | Descrição                                                                                                                                                                                                                                                                                                                         |
| :-------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **para apiserver**                      | O servidor API é um componente do plano de controle do Kubernetes que expõe a API do Kubernetes. O servidor API é o front-end do plano de controle do Kubernetes.                                                                                                                                                                 |
| **etcd**                                | Armazenamento de valores-chave consistente e altamente disponível usado como armazenamento de apoio do Kubernetes para todos os dados do cluster.                                                                                                                                                                                 |
| **para o agendador**                    | Observa pods recém-criados sem nó atribuído e seleciona um nó para execução.                                                                                                                                                                                                                                                      |
| **gerenciador de controlador kube**     | Executa processos do controlador. Logicamente, cada controlador é um processo separado, mas para reduzir a complexidade, todos eles são compilados em um único binário e executados em um único processo.                                                                                                                         |
| **gerenciador de controlador de nuvem** | Um componente do plano de controle do Kubernetes que incorpora lógica de controle específica da nuvem. O gerenciador de controlador de nuvem permite vincular seu cluster à API do seu provedor de nuvem e separa os componentes que interagem com essa plataforma de nuvem dos componentes que interagem apenas com seu cluster. |

###### 👷 Componentes do nó

Os componentes do nó são executados em cada nó, mantendo os pods em execução e fornecendo o ambiente de tempo de execução do Kubernetes.

| Componente                         | Descrição                                                                                                                                                                                                                           |
| :--------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Kubelet**                        | Um agente executado em cada nó do cluster. Isso garante que os contêineres estejam sendo executados em um pod.                                                                                                                      |
| **ser um procurador**              | Um proxy de rede executado em cada nó do cluster, implementando parte do conceito do Serviço Kubernetes.                                                                                                                            |
| **Tempo de execução do contêiner** | O software responsável pela execução de contêineres. Kubernetes oferece suporte a vários tempos de execução de contêiner: Docker, containerd, CRI-O e qualquer outra implementação do Kubernetes CRI (Container Runtime Interface). |

##### 📦 Objetos Kubernetes

Os objetos Kubernetes são entidades persistentes no sistema Kubernetes. O Kubernetes usa essas entidades para representar o estado do seu cluster.

| Objeto                   | Descrição                                                                                                                                                                                                         |
| :----------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Sob**                  | A menor e mais simples unidade no modelo de objeto do Kubernetes que você cria ou implanta. Um pod representa um conjunto de contêineres em execução no seu cluster.                                              |
| **Serviço**              | Uma forma abstrata de expor um aplicativo em execução em um conjunto de pods como um serviço de rede.                                                                                                             |
| **Volume**               | Um diretório que contém dados acessíveis aos contêineres em um pod.                                                                                                                                               |
| **Espaço para nome**     | Uma forma de dividir os recursos do cluster entre vários usuários.                                                                                                                                                |
| **Implantação**          | Fornece atualizações declarativas para pods e ReplicaSets. Você descreve um estado desejado em uma implantação e o controlador de implantação altera o estado real para o estado desejado em uma taxa controlada. |
| **Conjunto de réplicas** | Garante que um número especificado de réplicas de pod esteja em execução a qualquer momento.                                                                                                                      |
| **StatefulSet**          | Gerencia a implantação e o escalonamento de um conjunto de pods e fornece garantias sobre a ordem e a exclusividade desses pods.                                                                                  |
| **DaemonSet**            | Garante que todos (ou alguns) nós executem uma cópia de um pod.                                                                                                                                                   |
| **Trabalho**             | Cria um ou mais pods e garante que um número específico deles seja encerrado com êxito.                                                                                                                           |
| **CronJob**              | Cria trabalhos em uma programação baseada em tempo.                                                                                                                                                               |

#### ⎈ Elmo

Helm é um gerenciador de pacotes para Kubernetes.

Ele ajuda você a gerenciar aplicativos Kubernetes – Helm Charts ajuda a definir, instalar e atualizar até mesmo o aplicativo Kubernetes mais complexo.

##### 🎯 Conceitos-chave

| Conceito        | Descrição                                                                                                                                                                                          |
| :-------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Gráfico**     | Um pacote Helm. Ele contém todas as definições de recursos necessárias para executar um aplicativo, ferramenta ou serviço dentro de um cluster Kubernetes.                                         |
| **Repositório** | Um lugar onde os gráficos podem ser coletados e compartilhados.                                                                                                                                    |
| **Liberar**     | Uma instância de um gráfico em execução em um cluster Kubernetes. Muitas vezes, um gráfico pode ser instalado várias vezes no mesmo cluster. E cada vez que é instalado, um novo_liberar_é criado. |

##### 🚀 Comandos principais do Kubernetes

| Comando          | Descrição                           | Exemplo                                |
| :--------------- | :---------------------------------- | :------------------------------------- |
| `helm search`    | Procure gráficos em um repositório. | `helm search repo stable`              |
| `helm install`   | Instale um gráfico.                 | `helm install my-release stable/mysql` |
| `helm upgrade`   | Atualize uma versão.                | `helm upgrade my-release stable/mysql` |
| `helm uninstall` | Desinstale uma versão.              | `helm uninstall my-release`            |
| `helm list`      | Listar lançamentos.                 | `helm list`                            |

#### 🏗️ Outras plataformas de orquestração

##### OpenShift

**OpenShift**é uma família de produtos de software de conteinerização desenvolvidos pela Red Hat. Seu principal produto é o OpenShift Container Platform – uma plataforma local como serviço construída em torno de contêineres Docker orquestrados e gerenciados pelo Kubernetes com base no Red Hat Enterprise Linux.

##### Rancheiro

**Rancheiro**é uma pilha de software completa para equipes que adotam contêineres. Ele aborda os desafios operacionais e de segurança do gerenciamento de vários clusters Kubernetes em qualquer infraestrutura, ao mesmo tempo que fornece às equipes de DevOps ferramentas integradas para executar cargas de trabalho em contêineres.

##### Mesosphere DC/OS

**Mesosphere DC/OS**(o Sistema Operacional Datacenter) é um sistema operacional distribuído baseado no kernel de sistemas distribuídos Apache Mesos. Ele pode gerenciar várias máquinas em um datacenter ou nuvem como se fossem um único computador. Ele fornece uma maneira altamente elástica e escalonável de implantar aplicativos, serviços e infraestrutura de big data em recursos compartilhados.
...
A nuvem é um modelo de computação distribuída que fornece acesso sob demanda a um conjunto compartilhado de recursos de computação configuráveis ​​(redes, servidores, armazenamento, aplicativos e serviços) via Internet, provisionados e liberados rapidamente com esforço mínimo de gerenciamento.

#### Definição de computação em nuvem

Cloud Computing é o paradigma para entrega de serviços de TI através da internet, onde os recursos computacionais são alocados dinamicamente com base na demanda. Em vez de investir em infraestrutura local, os usuários e as organizações acessam o poder computacional, o armazenamento e os aplicativos por meio de modelos de assinatura ou de pagamento conforme o uso, com escalabilidade automática e gerenciamento centralizado.

> A computação em nuvem é a execução de cargas de trabalho em nuvens.

> Nuvens são ambientes de TI que abstraem, agregam e compartilham recursos escalonáveis ​​em uma rede.

> A computação em nuvem e as nuvens não são tecnologias em si.

> A computação em nuvem é uma ação, a execução de uma carga de trabalho em uma nuvem.

> Nuvens são ambientes, locais onde os aplicativos são executados.
...
Nuvem privada é um ambiente de computação em nuvem dedicado a uma única organização, hospedada localmente ou por um provedor terceirizado. Oferece segurança, controle e personalização aprimorados em comparação com nuvens públicas.

##### Nuvem Híbrida

A nuvem híbrida combina ambientes de nuvem pública e privada, permitindo que dados e aplicações sejam compartilhados entre eles. Essa abordagem oferece maior flexibilidade, escalabilidade e economia.

##### Multinuvem

Multinuvem refere-se ao uso de vários serviços de computação em nuvem de diferentes provedores dentro de uma única arquitetura. Essa estratégia ajuda a evitar a dependência de fornecedores, aumenta a redundância e permite que as organizações aproveitem os melhores recursos de cada provedor de nuvem.

#### IaaS, PaaS, SaaS

##### IaaS (infraestrutura como serviço)

IaaS fornece recursos de computação virtualizados sob demanda (máquinas virtuais, armazenamento de blocos e objetos, rede, GPUs). Até 2026, a IaaS evoluiu para incluir infraestrutura pronta para IA, provisionamento bare-metal automatizado e integração nativa com soluções de observabilidade, segurança e conformidade em tempo real.

##### PaaS (plataforma como serviço)

PaaS oferece uma plataforma gerenciada para desenvolvimento, implantação e operação de aplicativos sem gerenciar a infraestrutura subjacente. Em 2026, o PaaS inclui funções sem servidor, bancos de dados gerenciados, gateways de API, CI/CD nativo e arquiteturas orientadas a eventos e de microsserviços com escalonamento automático.
...
Apache CloudStack é uma plataforma de software de computação em nuvem de código aberto que fornece recursos IaaS para construir e gerenciar nuvens públicas e privadas.

CloudStack continua a ser uma escolha confiável para provedores de serviços e empresas, oferecendo recursos como multilocação, rede avançada (SDN) e suporte para vários hipervisores (KVM, XenServer, VMware) com integração aprimorada para orquestração de contêineres e computação de ponta.

#### Eucalipto

Eucalyptus é uma plataforma de computação em nuvem de código aberto que fornece recursos IaaS para construir e gerenciar nuvens privadas e híbridas. Ele foi projetado para ser compatível com APIs da Amazon Web Services (AWS), permitindo que os usuários executem aplicativos no Eucalyptus que já estão implantados na AWS.
...
-   [Orquestração de contêineres](<>)
    ...
    -   [Enxame Docker](https://docs.docker.com/engine/swarm/)
        ...
        -   [Pilha Docker Swarm](https://docs.docker.com/engine/swarm/stack-deploy/)
    -   [Brinque com Docker Swarm](https://labs.play-with-docker.com/)
    -   [Kubernetes](https://kubernetes.io/)
        -   [Kubernetes](https://kubernetes.io/)
        -   [Componentes](https://kubernetes.io/docs/concepts/overview/components/)
        -   [Arquitetura](https://kubernetes.io/docs/concepts/architecture/)
        -   [Site da Red Hat](https://www.redhat.com/pt-br/topics/containers/kubernetes-architecture)
...
-   [Documentos Openstack](<>)
    ...
    -   [O cenário OpenStack](https://www.openstack.org/software/)
