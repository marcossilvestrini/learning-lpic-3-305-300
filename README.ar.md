...
أقوم بإنشاء برنامج نصي بوويرشيل لتوفير المثيلات مع vagrant للمختبرات.  
إذا كان نظام SO الخاص بك هو windows، فيمكنك استخدام البرنامج النصي[app.ps1](./labs/app.ps1).

قم بتعيين بعض التكوينات لبيئتك:

-   ملف[Vagrantfile-الموضوع-351](./labs/vagrant/Vagrantfile-topic-351)
    ...
    -   ذاكرة vb = ""
    -   vb.cpus = ""
    -   مضمّن: "ifconfig eth1 &lt;your_public_ip_instanceX> قناع الشبكة 255.255.255.0 لأعلى"
    -   مضمّن: "المسار يضيف غيغاواط افتراضيًا &lt;your_public_gateway>"
...
يشير ترحيل V2V إلى عملية ترحيل جهاز افتراضي من برنامج Hypervisor إلى آخر.  
...
في الأساس، Xen Store عبارة عن قاعدة بيانات موزعة ذات قيمة أساسية تستخدم للاتصال ومشاركة المعلومات بين برنامج Xen Hypervisor والأجهزة الافتراضية (المعروفة أيضًا باسم المجالات) التي يديرها.
...
أجهزة HVM-DomUs عبارة عن أجهزة افتراضية تستخدم المحاكاة الافتراضية الكاملة، مما يسمح بتشغيل أنظمة التشغيل غير المعدلة. يوفر برنامج Hypervisor Xen محاكاة الأجهزة لوحدات DomUs هذه، مما يمكّنها من تشغيل أي نظام تشغيل يدعم بنية الأجهزة الأساسية. وفي حين أن هذا يوفر قدرًا أكبر من المرونة، إلا أنه يمكن أن يؤدي إلى زيادة في الحمل مقارنة بوحدات PV-DomUs.
