...
##### 🧪 مساحات أسماء المختبر

استخدم هذا البرنامج النصي للمختبر:[namespace.sh](scripts/container/namespace.sh)
...
استخدم هذا البرنامج النصي للمختبر:[cgroups.sh](scripts/container/cgroups.sh)

إخراج ذاكرة الحد الناعم:
...
استخدم هذا البرنامج النصي للمختبر:[capabilities.sh](scripts/container/capabilities.sh)

الإخراج:

![capabilities-lab](images/capabilities-lab.png)

#### 🛡 SecComp (وضع الحوسبة الآمن)

**ما هذا؟**

-   ميزة kernel Linux لتقييد أي syscalls (استدعاء النظام) يمكن استخدام العملية.
-   يستخدم عادة في الحاويات (مثل Docker) ، المتصفحات ، صناديق الرمل ، إلخ.

**كيف تعمل؟**

-   تتيح العملية ملف تعريف/مرشح SECCOMP.
-   الكنيل الكندل أو سجلات أو تقتل العملية إذا حاولت syscalls المحظورة.
-   تتم كتابة المرشحات بتنسيق BPF (مرشح حزمة بيركلي).
...
-   نظام التحكم الإلزامي للوصول (MAC) لتقييد ما يمكن للبرامج المحددة الوصول إليه.
-   ملفات التعريف هي النصية ، وموجهة نحو المسار ، وسهلة القراءة والتحرير.

**كيف تعمل؟**

-   يمكن أن يكون لكل ثنائي ملف تعريف يحدد الملفات والشبكات والقدرات المسموح بها - حتى كجذر!
-   من السهل التبديل بين أوضاع الشكوى والإنفاذ والمعوقين.
...
aa-genprof ، aa-logprof لإنشاء/تحديث ملفات تعريف
...
#### 🔒selinux (Linux المعزز بالأمان)

**ما هذا؟**

-   نظام MAC قوي للغاية للتحكم في الوصول إلى كل شيء: الملفات والعمليات والمستخدمين والمنافذ والشبكات والمزيد.
-   يستخدم الملصقات (السياقات) والسياسات التفصيلية.

**كيف تعمل؟**

-   كل شيء (العملية ، الملف ، المنفذ ، إلخ) يحصل على سياق أمان.
-   يتحقق kernel من كل إجراء مقابل قواعد السياسة.
...
-   Audit2allow ، Semanage ، CHCON (لإدارة السياسات/الملصقات)

-   سجلات: /var/log/audit/audit.log

-   السياسات:/etc/selinux/

#### 📋 جدول ملخص لأنظمة الأمان المشتركة

| نظام     | ركز                  | تعقيد | موقع السياسة                  | استخدام نموذجي        |
| -------- | -------------------- | ----- | ----------------------------- | --------------------- |
| Seccomp  | kernel syscalls      | واسطة | لكل عملية (عبر الكود/التكوين) | Docker ، صناديق الرمل |
| apparmor | لكل برنامج وصول      | سهل   | /etc/apparmor.d/              | ubuntu ، snap ، suse  |
| Selinux  | نظام MAC كامل النظام | متقدم | /etc/selinux/ + تسميات        | ريل ، فيدورا ، سنوس   |
...
##### Apparmor - تعزيز kernel لتحقيق البرامج في مجموعة محدودة من الموارد
