...
# تعلم LPIC-3 305-300 - اختبار ترجمة 02/07/2025
...
<details>
  ...
  <ol>
    ...
    <li>
      <a href="#topic-352">الموضوع 352: الافتراضية الحاوية</a>
      <ul>
        <li><a href="#topic-352.1">352.1 مفاهيم الافتراضية الحاوية</a></li>
...
قم بتبديل قالب *Vagrantfile-topic-xxx* وانسخ إلى ملف جديد باسم *Vagrantfile*
...
### لإعادة تشغيل الآلات الافتراضية
...
**مهم:**
*إذا قمت بإعادة تشغيل الآلات الافتراضية بدون Vagrant، فلن يتم تحميل المجلد المشترك بعد التمهيد.*

### استخدم PowerShell للتشغيل والإيقاف

إذا كنت تستخدم نظام Windows، فقد أنشأت نص PowerShell للتشغيل والإيقاف للآلات الافتراضية.
...
> 1. الحرية في دراسة كيفية عمل البرنامج، وتغييره بحيث يقوم بحساباتك كما تريد (الحرية 1).
> الوصول إلى الشيفرة المصدرية هو شرط مسبق لذلك.
...
* 🛠️ فهم الأنواع المختلفة من Hypervisors ومراقبي الآلات الافتراضية
...
يعمل مباشرة على الأجهزة الفيزيائية للمضيف، مما يوفر طبقة أساسية لإدارة الآلات الافتراضية دون الحاجة إلى نظام تشغيل مضيف.
...
* VMware ESXi: Hypervisor قوي ومستخدم على نطاق واسع في إعدادات المؤسسات.
* Microsoft Hyper-V: مدمج مع Windows Server، ويقدم أداءً قويًا وميزات إدارة.
* Xen: Hypervisor مفتوح المصدر يستخدمه العديد من مزودي خدمات السحابة.
* KVM (آلة افتراضية قائمة على النواة): مدمج في نواة Linux، مما يوفر أداءً عاليًا للأنظمة القائمة على Linux.
...
* QEMU (محاكي سريع): محاكي ومجسد مفتوح المصدر، غالبًا ما يستخدم بالاقتران مع KVM.

##### الاختلافات الرئيسية بين Hypervisors من النوع 1 و النوع 2
...
* الأداء:
  ...
  * قد تواجه Hypervisors من النوع 2 بعض تدهور الأداء بسبب العبء الناتج عن التشغيل فوق نظام تشغيل مضيف.
* الإدارة وسهولة الاستخدام:
  ...
  * من الأسهل تثبيت واستخدام Hypervisors من النوع 2، مما يجعلها مثالية للمستخدمين الأفراد والمشاريع الأصغر.

##### أنواع النقل

في سياق Hypervisors، التي هي تقنيات تستخدم لإنشاء وإدارة الآلات الافتراضية، فإن مصطلحات نقل P2V و V2V شائعة في بيئات الافتراضية.
تشير إلى عمليات نقل الأنظمة بين أنواع مختلفة من المنصات.

##### P2V - النقل من الفيزيائي إلى الافتراضي

يشير النقل P2V إلى عملية نقل خادم فعلي إلى آلة افتراضية. بمعنى آخر، يتم "تحويل" نظام التشغيل وتطبيقاته، التي تعمل على أجهزة فيزيائية مخصصة، إلى آلة افتراضية تعمل على Hypervisor (مثل VMware، Hyper-V، KVM، إلخ).

* مثال: لديك خادم فعلي يعمل بنظام Windows أو Linux، وترغب في نقله إلى بيئة افتراضية، مثل بنية تحتية سحابية أو خادم افتراضي داخلي.
  تتضمن العملية نسخ الحالة الكاملة للنظام، بما في ذلك نظام التشغيل، والسائقين، والبيانات، لإنشاء آلة افتراضية مكافئة يمكن أن تعمل كما لو كانت على الأجهزة الفيزيائية.
...
تستفيد HVM من الامتدادات العتادية التي توفرها وحدات المعالجة المركزية الحديثة لافتراضية العتاد، مما يمكّن من إنشاء وإدارة الآلات الافتراضية مع الحد الأدنى من عبء الأداء.

###### الخصائص الرئيسية لـ HVM

* 🖥️ **دعم العتاد**: يتطلب دعم وحدة المعالجة المركزية لامتدادات الافتراضية مثل Intel VT-x أو AMD-V.
* 🛠️ **افتراضية كاملة:** يمكن للآلات الافتراضية تشغيل أنظمة تشغيل ضيف غير معدلة، حيث يوفر Hypervisor محاكاة كاملة