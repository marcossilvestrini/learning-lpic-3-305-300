...
コンテナは、コード、ライブラリ、環境変数、および構成ファイルなど、必要な依存関係（孤立、ポータブル、再現性のあるユニット）に加えて、アプリケーションをパッケージ化する軽量仮想化テクノロジーです。
...
#### Rinuxコンテナの分離とセキュリティの比較

| テクノロジー         | 目的 /それが何をするか                                                 | 主な違い                                              | コンテナの例                                                      |
| -------------- | ------------------------------------------------------------ | ------------------------------------------------- | ----------------------------------------------------------- |
| **chroot🏠**   | プロセスの見かけのルートディレクトリを変更します。分離ファイルシステム。                         | 単純なファイルシステムの分離。します**ない**リソース、特権、またはシステムコールを制限します。 | Dockerは使用します`chroot`最小限の画像を構築するための内部的には、強い隔離のためではありません。     |
| **cgroups📊**  | プロセスのグループごとに、リソースの使用（CPU、メモリ、ディスクI/Oなど）を制御および制限します。          | カーネル機能;分離ではなく、細粒のリソース制御。                          | DockerとKubernetesはCgroupsを使用して、コンテナ/ポッドあたりのCPU/MEMを制限します。   |
| **名前空間🌐**     | システムリソースを分離する：PID、Mount、UTS、ネットワーク、ユーザー、IPC、時間。              | カーネル機能;さまざまな種類の分離を提供します。                          | 各コンテナは、独自の名前空間（PID、ネット、マウントなど）で実行されます。                      |
| **機能🛡️**      | ルートの特権を細かいユニットに分割します（例：Net\_管理者、sys\_管理者）。                   | すべてまたは無効なルート/非ルートよりも粒状。特定の特権をドロップまたは付与できます。       | Dockerコンテナは通常、機能を減らして実行されます（危険な機能をドロップします）。                 |
| **seccomp🧱**  | プロセスが作成できるシステム（ホワイトリスト/ブラックリスト）をフィルター/制限します。                 | 非常に焦点：カーネルシステムをブロックします。すべてのアクションをブロックすることはできません。  | Dockerのデフォルトのプロファイルは、危険なシステムをブロックします（例えば、`ptrace`,`mount`). |
| **Apparmor🐧** | 必須アクセス制御（MAC）フレームワーク：プロファイルを介してプログラムのファイル/ネットワークアクセスを制限します。  | プロファイルベース、SELINUXよりも管理しやすい。場合によっては細かくはありません。      | Ubuntuベースのコンテナは、多くの場合、コンテナプロセスプロファイルにApparmorを使用します。        |
| **selinux🔒**  | より複雑なMacフレームワーク、ラベルベース、非常に細かい粒子。ユーザー、プロセス、ファイルを閉じ込めることができます。 | Apparmorよりも強力で複雑です。 Fedora/Rhel/Centosで施行されています。  | OpenShift/KubernetesでRhelを使用して、Selinuxラベルを使用してポッドを分離します。    |

まとめ

-   Chroot：基本的な分離、リソース/セキュリティ保証なし。
-   cgroups：リソース制御、分離ではありません。
-   名前空間：カーネルリソースの「ビュー」を分離します。
-   機能：プロセスの特権を微調整します。
-   SECCOMP：システムコールサーフェスを制限します。
-   Apparmor/selinux：Root（Mac）としてであっても、プロセスが触れることができるものを制限します。

#### oci、runc、containred、cri、cri-o  - コンテナエコシステムにあるもの

##### 概要と役割

-   **OCI（オープンコンテナイニシアチブ）🏛️**

    オープンスタンダードを作成する基盤**コンテナ画像**そして**ランタイム**.

    _画像のフォーマット、保存、およびコンテナの開始/停止方法（ランタイム仕様）を定義します。_
-   **⚙️runc**

    OCIランタイム仕様に従ってコンテナを実行できる、普遍的な、低レベルの軽量CLIツール。

    _画像 +構成を実際の実行中のLinuxコンテナに変える「エンジン」。_
-   **containerd🏋️**

    完全なコンテナライフサイクルを管理するためのコアコンテナランタイムデーモン：**画像を引っ張り、ストレージの管理、実行中のコンテナ**（runcを呼び出します）、ネットワーキングプラグインなど。

    _Docker、Kubernetes、nerdctl、およびその他のツールがメインコンテナランタイムバックエンドとして使用しています。_
-   **CRI（コンテナランタイムインターフェイス）🔌**

    Kubernetes固有のGRPC APIを使用して、Kubernetesをコンテナのランタイムに接続します。

    _Kubernetes以外では使用していませんが、K8SがContainerd、Cri-Oなどと話すことができます。_
-   **cri-o🥤**

    軽量のKubernetesに焦点を当てたランタイム**のみ**フードの下でRuncを使用して、OCIコンテナを実行します。

    _主にKubernetesで使用されていますが、オープン標準に焦点を当てた最小限のコンテナランタイムを構築する方法を示しています。_

##### 🏷️比較表：OCI、RUNC、CONTAINERD、CRI、CRI-O

| 成分             | Emoji | それは何ですか？              | 誰がそれを使用しますか？                        | 使用の例                                            |
| -------------- | ----- | --------------------- | ----------------------------------- | ----------------------------------------------- |
| **oci**        | 🏛️   | 標準/仕様                 | Docker、Podman、cri-o、containred、runc | 画像/コンテナがツール間で互換性があることを保証します                     |
| **runc**       | ⚙️    | コンテナランタイム（CLI）        | Containerd、Cri-O、Docker、Podman      | バンドルからコンテナを直接実行します（例：`runc run`)                |
| **containerd** | 🏋️   | コンテナランタイムデーモン         | Docker、Kubernetes、nerdctl           | ストレージ/ネットワークを管理する画像の引っ張りを処理し、RUNCを介してコンテナを起動します |
| **CRI**        | 🔌    | K8Sランタイムインターフェイス（API） | Kubernetesのみ                        | kubeletがcontainerd/cri-oに話しかけましょう               |
| **それを批判します**   | 🥤    | K8Sの軽量コンテナランタイム       | Kubernetes、OpenShift                | K8Sコンテナエンジンとして使用                                |
...
-   **画像の構築：**
    ...
    任意のツール（Docker、Podman、Buildah）は、**OCI画像仕様**したがって、どこにでも互換性があります。
-   **実行中のコンテナ：**

    PodmanとDockerの両方が最終的に使用されます**runc**（ContainRDを介して、または直接）コンテナを作成します。
-   **多くのコンテナの管理：**

    **containerd**単独で使用できます（経由`ctr`または`nerdctl`）またはDockerおよびKubernetesのバックエンドとして。
-   **プラグアンドプレイランタイム：**

    に感謝します**oci**、画像の構築方法や管理方法を変更せずに、RUNCを別のOCIに準拠したランタイム（VMS用のKataコンテナ、サンドボックス用のGVISORなど）に交換できます。
...
-   **Docker**：ユーザー151→containerd→runc
-   **サブマン**：ユーザー151→Runc
-   **Kubernetes**：kubelet（cri）→containerdまたはcri-o→runc

* * *

##### 🧠概要

-   **oci**=画像/ランタイムの共通言語（標準/仕様）
-   **runc**=コンテナプロセスを作成および管理する実際のツール
-   **containerd**=画像、コンテナ、ライフサイクルを管理するフル機能のデーモン
-   **CRI**= Kubernetesのみ、ランタイムをプラグ可能にする
-   **それを批判します**= OCI標準とRuncに基づいて構築されたKubernetesに焦点を当てた軽量のランタイム
...
RUNCラボでは、このスクリプトを使用できます。[runc.sh](scripts/container/runc.sh)

![runc](images/runc-lab.png)

##### 🧪LabContainerd

runc containerdの場合、このスクリプトを使用できます。[containerd.sh](scripts/container/container.sh)

![containerd](images/containerd-lab.png)

* * *

#### podman、buildah、scopeo、openvz、Crun＆Kataコンテナ - ファーストトラック

* * *

##### 🐳**サブマン**

-   **それは何ですか？**Docker CLIと互換性のあるコンテナマネージャーですが**デーモンレス**そして実行できます**ルートレス**.
-   **使用：**コンテナとポッドを作成、実行、停止、および検査します。
-   **ハイライト：**マルチユーザーにとってより安全なセントラルデーモンは、SystemDと統合されていません。
-   [詳細](<>)
...
-   **それは何ですか？**にツール**コンテナ画像を構築および操作します**（oci/docker）デーモンなし。
-   **使用：**CI/CDパイプラインまたはスクリプトの画像の構築。
-   **ハイライト：**ポッドマンがフードの下で使用する軽量で根のないサポート。
-   [詳細](https://www.redhat.com/en/topics/containers/what-is-buildah)

* * *

##### 🔭**範囲**

-   **それは何ですか？**ユーティリティ**コンテナ画像を検査、コピー、および移動します**レジストリ間**引っ張ったり実行したりせずに**彼ら。
-   **使用：**画像を移動し、署名とメタデータを確認します。
-   **ハイライト：**デーモンはありません。自動化とセキュリティに最適です。
-   [詳細](<>)
...
-   **それは何ですか？****コンテナベースの仮想化**Linuxのソリューション（最新のコンテナツール前の日付）。
-   **使用：**同じカーネルを共有する軽量VP（仮想プライベートサーバー）。
-   **ハイライト：**非常に効率的ですが、VM（カーネルを共有）よりも孤立していません。
-   [詳細](https://en.wikipedia.org/wiki/OpenVZ)

* * *

##### ⚡**クラン**

-   **それは何ですか？**c（goではない）で記述されたコンテナの超高速で最小限のOCIランタイム。
-   **使用：**最小限のオーバーヘッドでコンテナを実行します。
-   **ハイライト：**Runcよりも速くて軽量で、一部のシステムのPodmanのデフォルト。
-   [詳細](https://www.redhat.com/sysadmin/introduction-crun)

* * *

##### 🛡️**ワードコンテナ**

-   **それは何ですか？**コンテナとVMを組み合わせたオープンソースプロジェクト：各コンテナは軽量マイクロVMで実行されます。
-   **使用：**敏感なワークロードまたはマルチテナント環境の強力な分離。
-   **ハイライト：**VMグレードのセキュリティ、ほぼコンテナーパフォーマンス。
-   [詳細](https://katacontainers.io/)

* * *

##### 📊**比較表**

| プロジェクト      | カテゴリ       | 分離             | デーモン？ | 主な用途        | ルートレス | メモ                    |
| ----------- | ---------- | -------------- | ----- | ----------- | ----- | --------------------- |
| **サブマン**    | オーケストレーション | 容器             | いいえ   | コンテナを管理します  | はい    | DockerのようなCLI         |
| **buildah** | 建てる        | n/a            | いいえ   | 画像を作成します    | はい    | CI/CDの場合、コンテナは実行されません |
| **範囲**      | 画像転送       | n/a            | いいえ   | 画像を移動/確認します | はい    | コンテナの実行はありません         |
| **openvz**  | 仮想化        | コンテナ/VP        | はい    | 軽量VP        | いいえ   | カーネル共有、レガシーテクノロジー     |
| **クラン**     | OCIランタイム   | 容器             | いいえ   | 高速コンテナランタイム | はい    | Runcよりも速い             |
| **ワードコンテナ** | ランタイム/VM   | コンテナあたりのマイクロフム | いいえ   | 強い隔離        | はい    | VMレベルのセキュリティ          |

* * *

##### ☑️**簡単な要約**

-   **ポッドマン：**モダンなデーモンレスドッカーの代替。
-   **Buildah：**画像を作成し、コンテナを実行しません。
-   **スコピアン：**画像を移動/検査し、それらを実行することはありません。
-   **OpenVZ：**レガシーコンテナベースのVP。
-   **残酷な：**超高速で軽量のOCIランタイム。
-   **言う：**VMレベルの分離を備えたコンテナ。
...
-   [リチャード・ストールマン's](http://www.stallman.org/)
-   [gnu](<>)
...
-   [仮想化とコンテナ化](<>)
    ...
    -   [DockerのSecCompプロファイル](https://docs.docker.com/engine/security/seccomp/)
    -   [DockerのApparmorプロファイル](https://docs.docker.com/engine/security/apparmor/)
    -   [selinux](https://pt.wikipedia.org/wiki/SELinux)
    -   [Apparmor Selinuxの比較](https://www.redhat.com/en/blog/apparmor-selinux-isolation)
    -   [runc](https://www.docker.com/blog/runc/)
    -   [Runc Github](https://github.com/opencontainers/runc)
    -   [oci](https://opencontainers.org/about/overview/)
    -   [CRI](https://kubernetes.io/docs/concepts/architecture/cri/)
    -   [それを批判します](https://cri-o.io/)
    -   [containerd](https://containerd.io/)
    -   [サブマン](https://www.redhat.com/pt-br/topics/containers/what-is-podman)
    -   [範囲](https://www.redhat.com/pt-br/topics/containers/what-is-skopeo)
    -   [buildah](https://www.redhat.com/en/topics/containers/what-is-buildah)
    -   [openvz](https://openvz.org/)
    -   [クラン](https://www.redhat.com/en/blog/introduction-crun)
    -   [言う](https://katacontainers.io/)
-   [OpenStackドキュメント](<>)
