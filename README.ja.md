...
# LEARNING LPIC-3 305-300 - テスト翻訳 02/07/2025
...
> このプロジェクトは、学生や専門家がGNULinuxの主要な概念とフリーソフトウェアを学ぶのを助けることを目的としています。
...
> Vagrantを使用してマシンを立ち上げ、この記事のラボや実践コンテンツを実行します。
> Vagrantフォルダに、学習用の環境をアップロードするために必要なVagrantfileを公開しました。
...
ファイル [configs/network](configs/network/) でネットワーク設定をカスタマイズします。
...
*Vagrantfile-topic-xxx* テンプレートを切り替え、新しいファイル名 *Vagrantfile* でコピーします。
...
**説明:** 候補者は、仮想化の一般的な概念、理論、および用語を知り、理解している必要があります。これには、Xen、QEMU、およびlibvirtの用語が含まれます。
...
* VMware ESXi: 企業環境で広く使用される堅牢なハイパーバイザー。
* Microsoft Hyper-V: Windows Serverに統合されており、強力なパフォーマンスと管理機能を提供します。
* Xen: 多くのクラウドサービスプロバイダーによって使用されるオープンソースのハイパーバイザー。
* KVM (Kernel-based Virtual Machine): Linuxカーネルに統合されており、Linuxベースのシステムに高性能を提供します。
...
* 🐢 通常、ホストOSからの追加のオーバーヘッドのためにタイプ1ハイパーバイザーよりも効率が低いです。

###### タイプ2の例

* VMware Workstation: 単一のデスクトップで複数のオペレーティングシステムを実行するための強力なハイパーバイザー。
* Oracle VirtualBox: 柔軟性と使いやすさで知られるオープンソースのハイパーバイザー。
* Parallels Desktop: MacユーザーがWindowsや他のオペレーティングシステムをmacOSと一緒に実行するために設計されています。
* QEMU (Quick EMUlator): オープンソースのエミュレーターおよび仮想化ツールで、KVMと一緒に使用されることがよくあります。

##### タイプ1とタイプ2ハイパーバイザーの主な違い

* デプロイメント環境:
  * タイプ1ハイパーバイザーは、ハードウェアとの直接的な相互作用と高性能のため、データセンターや企業環境で一般的に展開されます。
...
ハイパーバイザーの文脈では、仮想マシンを作成および管理するために使用される技術であり、P2V移行とV2V移行という用語が仮想化環境で一般的です。
これらは、異なるタイプのプラットフォーム間でシステムを移行するプロ