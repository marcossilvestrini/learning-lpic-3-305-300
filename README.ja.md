...
-   [バーチャルボックス](https://blogs.vmware.com/workstation/2024/05/vmware-workstation-pro-now-available-free-for-personal-use.html)
...
ラボ用に vagrant を使用してインスタンスをプロビジョニングするためのスクリプト Powershell を作成しています。  
SO システムが Windows の場合は、スクリプトを使用できます[app.ps1](./labs/app.ps1).

環境に合わせていくつかの構成を設定します。

-   ファイル[Vagrantfile-topic-351](./labs/vagrant/Vagrantfile-topic-351)
    ...
    -   vb.memory = ""
    -   vb.cpus = ""
    -   インライン: "ifconfig eth1 &lt;your_public_ip_instanceX> ネットマスク 255.255.255.0 アップ"
    -   インライン: "ルート追加デフォルト GW &lt;your_public_gateway>"
...
-   例: Windows または Linux システムを実行している物理サーバーがあり、それをクラウド インフラストラクチャや内部仮想化サーバーなどの仮想環境に移動したいと考えています。  
    このプロセスには、オペレーティング システム、ドライバー、データを含むシステム状態全体をコピーして、物理ハードウェア上であるかのように実行できる同等の仮想マシンを作成することが含まれます。
...
-   **イベントと通知:**Xen Store はイベント通知もサポートしています。 Xen ストア内の特定のキーまたは値が変更されると、関係するドメインにこれ​​らの変更に対応するよう通知できます。これは、リソースの監視と管理に役立ちます。
