...
: <<'MULTILINE-COMMENT'
	📝 Script Name: Packer-storage-lab.sh
	📚 Description: Prepare a secondary disk for Packer storage labs (LVM, Btrfs, ZFS) 
...
cleanup_all() {
  ...
  sudo vgchange -an Packer-lvm 2>/dev/null || true
  sudo lvchange -an Packer-lvm 2>/dev/null || true
  # If a previous pool name was used, export/destroy silently
  sudo zpool export Packer-zfs 2>/dev/null || true
  sudo zpool destroy Packer-zfs 2>/dev/null || true
...
summary() {
  echo "======= 📊 Packer Storage Lab Summary ======="
  lsblk -f
...
if [[ -b "${DISK}1" ]]; then
  ...
  sudo vgcreate -ff -y Packer-lvm "${DISK}1"
fi
...
if [ -b "${DISK}2" ]; then
	...
	sudo mkfs.btrfs -f -L Packer-btrfs "${DISK}2"
	sudo partprobe "$DISK"
	sudo udevadm settle
	sleep 1
else
...
if [[ -b "${DISK}3" ]]; then
  if command -v zpool >/dev/null 2>&1; then
    log "Creating ZFS pool 'Packer-zfs' on ${DISK}3..."
    sudo zpool create -f Packer-zfs "${DISK}3" || warn "zpool create returned non-zero (pool may already exist)."
    # Optional: export immediately to keep the disk free for future labs
    # sudo zpool export Packer-zfs 2>/dev/null || true
    sudo zpool import Packer-zfs 2>/dev/null || true
