...
: <<'MULTILINE-COMMENT'
	📝 Script Name: Kubernetes-storage-lab.sh
	📚 Description: Prepare a secondary disk for Kubernetes storage labs (LVM, Btrfs, ZFS) 
...
cleanup_all() {
  ...
  sudo vgchange -an Kubernetes-lvm 2>/dev/null || true
  sudo lvchange -an Kubernetes-lvm 2>/dev/null || true
  # If a previous pool name was used, export/destroy silently
  sudo zpool export Kubernetes-zfs 2>/dev/null || true
  sudo zpool destroy Kubernetes-zfs 2>/dev/null || true
...
summary() {
  echo "======= 📊 Kubernetes Storage Lab Summary ======="
  lsblk -f
...
if [[ -b "${DISK}1" ]]; then
  ...
  sudo vgcreate -ff -y Kubernetes-lvm "${DISK}1"
fi
...
if [ -b "${DISK}2" ]; then
	...
	sudo mkfs.btrfs -f -L Kubernetes-btrfs "${DISK}2"
	sudo partprobe "$DISK"
	sudo udevadm settle
	sleep 1
else
...
if [[ -b "${DISK}3" ]]; then
  if command -v zpool >/dev/null 2>&1; then
    log "Creating ZFS pool 'Kubernetes-zfs' on ${DISK}3..."
    sudo zpool create -f Kubernetes-zfs "${DISK}3" || warn "zpool create returned non-zero (pool may already exist)."
    # Optional: export immediately to keep the disk free for future labs
    # sudo zpool export Kubernetes-zfs 2>/dev/null || true
    sudo zpool import Kubernetes-zfs 2>/dev/null || true
