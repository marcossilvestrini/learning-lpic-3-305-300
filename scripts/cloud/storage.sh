...
: <<'MULTILINE-COMMENT'
	📝 Script Name: cloud-storage-lab.sh
	📚 Description: Prepare a secondary disk for Cloud storage labs (LVM, Btrfs, ZFS) 
...
cleanup_all() {
  ...
  sudo vgchange -an cloud-lvm 2>/dev/null || true
  sudo lvchange -an cloud-lvm 2>/dev/null || true
  # If a previous pool name was used, export/destroy silently
  sudo zpool export cloud-zfs 2>/dev/null || true
  sudo zpool destroy cloud-zfs 2>/dev/null || true
...
summary() {
  echo "======= 📊 Cloud Storage Lab Summary ======="
  lsblk -f
...
if [[ -b "${DISK}1" ]]; then
  ...
  sudo vgcreate -ff -y cloud-lvm "${DISK}1"
fi
...
if [ -b "${DISK}2" ]; then
	...
	sudo mkfs.btrfs -f -L cloud-btrfs "${DISK}2"
	sudo partprobe "$DISK"
	sudo udevadm settle
	sleep 1
else
...
if [[ -b "${DISK}3" ]]; then
  if command -v zpool >/dev/null 2>&1; then
    log "Creating ZFS pool 'cloud-zfs' on ${DISK}3..."
    sudo zpool create -f cloud-zfs "${DISK}3" || warn "zpool create returned non-zero (pool may already exist)."
    # Optional: export immediately to keep the disk free for future labs
    # sudo zpool export cloud-zfs 2>/dev/null || true
    sudo zpool import cloud-zfs 2>/dev/null || true
