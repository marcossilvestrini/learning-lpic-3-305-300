...
# -------------------------------------------------
# Prepare packages for zfs.sh and storage.sh
# -------------------------------------------------

# # Ensure 'contrib non-free non-free-firmware' are present on all 'deb ... bookworm main' lines
# echo "-------------------------------------------------------"
# echo "Ensuring 'contrib non-free non-free-firmware' components in apt sources..."
# echo "-------------------------------------------------------"
# FILE="/etc/apt/sources.list"
# if ! grep -q "non-free-firmware" "$FILE" ]; then
#     echo "Adding 'contrib non-free non-free-firmware' components to apt sources..."
#     sudo sed -i -E 's/^(deb\s+[^ ]+\s+bookworm\s+main)(.*)$/\1 contrib non-free non-free-firmware\2/' "$FILE"
# fi
...
# # Preseed Postfix to avoid prompts
